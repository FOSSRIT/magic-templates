module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      font: {
        files: [{expand: true, cwd: 'style/fonts', src:['**'], dest: '../style/fonts'}]
      },
      img: {
        files : [{expand: true, cwd: 'img', src: ['**'], dest: '../img'}]
      },
      js: {
        files : [{expand: true, cwd: 'js', src: ['**'], dest: '../js'}]
      },
      module_js: {
        files: [{expand: true, cwd: 'modules', src: ['**/js/*.js'], dest: '../js/modules/'}]
        ...
        //     module = path.split('/')[1];
        //     array.push({expand: true, cwd: path, src: ['**'], dest: '../js/modules/'});
        //   });
        //   return array;
        // }
      }
    ...
    jade: {
      index: {
        options: jadedebug,
        files: [{expand: true, cwd: './', src: ['*.jade'], dest: '../', ext: '.html', flatten: true }]
      },
      pages: {
        options: jadedebug,
        files: [{expand: true, cwd: 'pages', src: '*.jade', dest: '../', ext: '.html', flatten: true}]
      }
    },
    ...
    cssmin: {
      dist: {
        src: ['../style/style.css', '../style/modules/*.css'],
